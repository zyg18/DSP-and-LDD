...
int main(int argc, char** argv)
{
	...
	edge_sum = edge_sum/2;                       //之前为了方便重复计数
	while (degreelist[start_list]!=-1) {
		//int i = lists[0].next;                    //i为开头列表的序号
		//int k = lists[i].idx;                     //k为度列表里的最后一个点的序号 
		...
		for (int p = A.outerIndexPtr()[k]; p<A.outerIndexPtr()[k+1]; p++) { //decrement degrees of k's neighbors
			...
			//int i1 = lists[i].prev;
			int i1 = prev[degree[neighbour_index]];            //neighbour的前一个度节点列表
